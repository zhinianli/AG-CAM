...
def compare(start, step, TP, P, T, input_type, threshold, predict_folder, gt_folder, name_list, num_cls=21):
...
def do_python_eval(predict_folder, gt_folder, name_list, num_cls=21, input_type='png', threshold=1.0, printlog=False):
    ...
    p_list = []
    for i in range(8):
        p = multiprocessing.Process(target=compare, args=(i, 8, TP, P, T, input_type, threshold, predict_folder, gt_folder, name_list))
        p.start()
        p_list.append(p)
    for p in p_list:
        p.join()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--list", default='../VOCdevkit/VOC2012/ImageSets/Segmentation/train.txt', type=str)
    parser.add_argument("--predict_dir", default='data/cam', type=str)
    ...
    parser.add_argument('--comment', default='cam', type=str)
    ...
    if not args.curve:
        ...
        for i in range(45, 60):
            ...
            print('%d/ background score: %.3f\tmIoU: %.3f%%' % (i, t, loglist['mIoU']))
