...
def get_img_path_coco(img_name, voc12_root):
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~ coco ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def load_img_name_list_coco(dataset_path):
    img_gt_name_list = open(dataset_path).read().splitlines()
...
class VOC12ImageDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, voc12_root, transform=None):
        ...
        # self.img_name_list = load_img_name_list_coco(img_name_list_path)
    ...
    def __getitem__(self, idx):
        ...
        image = np.asarray(imageio.imread(get_img_path(name, self.voc12_root)))

        if self.transform:
            img = self.transform(img)

        valid_mask = torch.zeros((21, 512, 512))
        image, box = imutils.my_random_crop(image, 512, 0)
        valid_mask[:, box[0]:box[1], box[2]:box[3]] = 1

        return name, img, valid_mask
...
class VOC12ClsDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        name, img, valid_mask = super().__getitem__(idx)

        label = torch.from_numpy(self.label_list[idx])

        return name, img, valid_mask, label
...
class VOC12ClsDatasetMSF(VOC12ClsDataset):
    ...
    def __getitem__(self, idx):
        name, img, valid_mask, label = super().__getitem__(idx)
        ms_img_list = []
        for s in [512]:
            target_size = (s, s)
...
class VOC12AffDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        name, img, _ = super().__getitem__(idx)
        ...
        for joint_transform, img_transform, label_transform \
            ...
            if joint_transform:
                img_label = np.concatenate((img, np.expand_dims(label_la, axis=2), np.expand_dims(label_ha, axis=2)),
                                           axis=-1)
